...
function imax_setup() {
	 ...
	 * replace to change 'imax' to the name of your theme in all
	 * template files.
	 */
	load_theme_textdomain( 'imax', get_template_directory() . '/languages' );
	...
	register_nav_menu( 'primary', __( 'Navigation Menu', 'imax' ) );
	 ...
	 */
	add_image_size( 'slider-thumb', 1600, 580, true ); //(cropped)	
	add_image_size( 'single-thumb', 1200, 480, true ); //(cropped)
	add_image_size( 'tx-medium', 600, 480, true ); //(cropped)	
...
function imax_fonts_url() {
	...
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'imax' );

	/* Translators: If there are characters in your language that are not
	 * supported by Roboto, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$roboto = _x( 'on', 'Roboto font: on or off', 'imax' );
...
function imax_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'imax' ), max( $paged, $page ) );
...
function imax_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Main Widget Area', 'imax' ),
		'id'            => 'sidebar-1',
		'description'   => __( 'Appears in the footer section of the site.', 'imax' ),
	...
	register_sidebar( array(
		'name'          => __( 'Main Sidebar Widget Area', 'imax' ),
		'id'            => 'sidebar-2',
		'description'   => __( 'Appears on posts and pages in the sidebar.', 'imax' ),
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>',
...
function imax_paging_nav() {
    ...
    <?php
		...
		$args = array(
			...
			'type' => 'list',
			'prev_text' => '<span class="text">&laquo; ' . __( 'Previous', 'imax' ) . '</span>',
			'next_text' => '<span class="text">' . __( 'Next', 'imax' ) . ' &raquo;</span>',
			'add_args' => false					
		);
	?>				    
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'imax' ); ?></h1>
		<div class="nav-links">
...
function imax_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'imax' ); ?></h1>
		<div class="nav-links">

			<?php previous_post_link( '%link', _x( '<span class="meta-nav">&larr;</span> %title', 'Previous post link', 'imax' ) ); ?>
			<?php next_post_link( '%link', _x( '%title <span class="meta-nav">&rarr;</span>', 'Next post link', 'imax' ) ); ?>
...
function imax_entry_meta() {
	if ( is_sticky() && is_home() && ! is_paged() )
		echo '<span class="featured-post">' . __( 'Sticky', 'imax' ) . '</span>';

	if ( ! has_post_format( 'link' ) && 'post' == get_post_type() )
		imax_entry_date();

	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( __( ', ', 'imax' ) );
	if ( $categories_list ) {
		echo '<span class="categories-links">' . $categories_list . '</span>';
	}

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'imax' ) );
	if ( $tag_list ) {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
			...
			esc_attr( sprintf( __( 'View all posts by %s', 'imax' ), get_the_author() ) ),
			get_the_author()
		);
	}
}
...
function imax_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) )
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'imax' );
	else
		$format_prefix = '%2$s';

	$date = sprintf( '<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>',
		esc_url( get_permalink() ),
		esc_attr( sprintf( __( 'Permalink to %s', 'imax' ), the_title_attribute( 'echo=0' ) ) ),
...
function imax_excerpt_more($more) {
       global $post;
	return '<a class="moretag" href="'. get_permalink($post->ID) . '">'. __( 'Read More...', 'imax' ). '</a>';
}
